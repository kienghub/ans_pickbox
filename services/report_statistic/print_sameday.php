  ...
  <body onload="printThis('invoice')" style="padding:20px" id="invoice">
       <?php 
          ...
          if($_GET['start_date']=="" & $_GET['end_date']==""){
               ...
               $params="AND ans_items.sendDate BETWEEN '$st_date' AND '$end_date'";
          }
     // SUM QTY RECEIVE
     ...
     $sum_qty_for_next_day2=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=1 AND ans_items.status_id=5 $params");
     ...
     $sum_packagePrice_for_next_day=$_sql($main_db,"SELECT sum(ans_item_next_day.packagePrice) as packagePrice FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=0 AND ans_items.status_id=5 $params");
     ...
     function _sumPriceFromProvince($x){
      ...
      function _sumWeightForBranch($x){
          ...
          global $params;
          $query=mysqli_query($main_db,"SELECT sum(ans_item_next_day.weight) as weight FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.status_id=5 AND ans_items.branch_id='$x' $params");
          $res=$_assoc($query);
          echo number_format($res['weight']);
          mysqli_close($main_db);
     }
     ...
     function _percentFromProvince($x){
        ...
        function _countFromBranch($x){
          ...
          $sum_qty_for_next_day=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=0 AND ans_items.status_id=5 AND ans_item_next_day.destinationStateID='$x' $params");
      ...
      function qty_charge_on_shop1($x){
          ...
          global $params;
          $sum_qty_for_next_day=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=1 AND ans_items.status_id=5 AND ans_item_next_day.destinationStateID='$x' $params");
           $countRecQty=$_assoc($sum_qty_for_next_day);
          echo number_format($countRecQty['qty']);
          mysqli_close($main_db);
     }
       ...
       function branch_qty_charge_on_shop0($x){
          ...
          global $params;
          $sum_qty_for_next_day=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=0 AND ans_items.status_id=5 AND ans_items.branch_id='$x' $params");
           $countRecQty=$_assoc($sum_qty_for_next_day);
          echo number_format($countRecQty['qty']);
          mysqli_close($main_db);
     }
       ...
       function branch_qty_charge_on_shop1($x){
          ...
          global $params;
          $sum_qty_for_next_day=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=1 AND ans_items.status_id=5 AND ans_items.branch_id='$x' $params");
           $countRecQty=$_assoc($sum_qty_for_next_day);
          echo number_format($countRecQty['qty']);
          mysqli_close($main_db);
     }
...
?>
       ...
       <table width="100%" style="text-align:left;margin-left:30px">
            <tr>
                 <th colspan="2">
                      <center>
                           <h3 style="color:black!important">ລາຍງານພັດສະດຸທີ່ສົ່ງສຳເລັດ<?php echo $title ?>
                           </h3>
                      </center>
                 </th>
            </tr>
       ...
       <ul>
            <li class="p-2">
                 ເຄື່ອງທີ່ສົ່ງສຳເລັດທັງໝົດ
                 <?php echo @number_format($sumRecQty['rec_qty'])?> ອັນ
            </li>
            <li class="p-2">
                 ມູນຄ່າເຄື່ອງທີ່ສົ່ງສຳເລັດທັງໝົດ
                 <?php echo @number_format($sumRecPrice['rec_price'])?> ກີບ
            </li>
            <li class="p-2">
                 ນ້ຳໜັກ
                 <?php echo number_format($sumRecWeight['weight'])?>
                 kg
            </li>
            <li class="p-2">
       ...
       <table width="100%" id="table">
          ...
          foreach ($query_branch_in_state as $for) {?>
            <tr>
                 ...
                 <td style="text-align:right"><?php branch_qty_charge_on_shop0($for['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_price_charge_on_shop0($for['branch_id']) ?></td>
                 <td style="text-align:right"><?php _sumWeightForBranch($for['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_qty_charge_on_shop1($for['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_price_charge_on_shop1($for['branch_id']) ?></td>
            </tr>
            <?php $x++; } $i++;}?>
       </table>
       <h2># ພາກກາງ</h2>
       <table width="100%" id="table">
          ...
          $x=1;
          $pro_id=$state['id_state'];
          $query_branch_in_states =mysqli_query($main_db,"SELECT count(id_list) as total,branch_id FROM ans_item_next_day 
          LEFT JOIN ans_items ON ans_items.id_item=ans_item_next_day.item_id WHERE ans_item_next_day.destinationStateID='$pro_id'
          GROUP BY branch_id ORDER BY total DESC");
          mysqli_close($con);
          foreach ($query_branch_in_states as $result) {?>
            <tr>
                 <td></td>
                 ...
                 <td style="text-align:right"><?php branch_qty_charge_on_shop0($result['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_price_charge_on_shop0($result['branch_id']) ?></td>
                 <td style="text-align:right"><?php _sumWeightForBranch($result['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_qty_charge_on_shop1($result['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_price_charge_on_shop1($result['branch_id']) ?></td>
            </tr>
            <?php $x++; } $i++;}?>
       </table>
       <h2># ພາກໃຕ້</h2>
       <table width="100%" id="table">
          ...
          foreach ($query_branch_in_state as $resultset) {?>
            <tr>
                 ...
                 <td style="text-align:right"><?php branch_qty_charge_on_shop0($resultset['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_price_charge_on_shop0($resultset['branch_id']) ?></td>
                 <td style="text-align:right"><?php _sumWeightForBranch($resultset['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_qty_charge_on_shop1($resultset['branch_id']) ?></td>
                 <td style="text-align:right"><?php branch_price_charge_on_shop1($resultset['branch_id']) ?></td>
            </tr>
            <?php $x++; } $i++;}?>
       </table>
