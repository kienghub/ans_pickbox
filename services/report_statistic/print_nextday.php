     ...
     function _percentFromProvince($x){
        ...
        function _countFromBranch($x){
          ...
          $sum_qty_for_next_day=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=0 AND ans_items.status_id=3 AND ans_item_next_day.destinationStateID='$x' $params");
      ...
      function qty_charge_on_shop1($x){
          ...
          global $params;
          $sum_qty_for_next_day=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=1 AND ans_items.status_id=3 AND ans_item_next_day.destinationStateID='$x' $params");
           $countRecQty=$_assoc($sum_qty_for_next_day);
          echo number_format($countRecQty['qty']);
          mysqli_close($main_db);
     }
       ...
       function branch_qty_charge_on_shop0($x){
          ...
          global $params;
          $sum_qty_for_next_day=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=0 AND ans_items.status_id=3 AND ans_items.original_branch_id='$x' $params");
           $countRecQty=$_assoc($sum_qty_for_next_day);
          echo number_format($countRecQty['qty']);
          mysqli_close($main_db);
     }
       ...
       function branch_qty_charge_on_shop1($x){
          ...
          global $params;
          $sum_qty_for_next_day=$_sql($main_db,"SELECT count(id_list) as qty FROM ans_item_next_day 
                                        LEFT JOIN ans_items ON ans_items.id_item = ans_item_next_day.item_id WHERE ans_items.charge_on_shop=1 AND ans_items.status_id=3 AND ans_items.original_branch_id='$x' $params");
           $countRecQty=$_assoc($sum_qty_for_next_day);
          echo number_format($countRecQty['qty']);
          mysqli_close($main_db);
     }
...
?>
       ...
       <table width="100%" style="text-align:left;margin-left:30px">
            <tr>
                 <th colspan="2">
                      <center>
                           <h3 style="color:black!important">ລາຍງານພັດສະດຸເຂົ້າສາງ<?php echo $title ?>
                           </h3>
                      </center>
                 </th>
            </tr>
